...
public sealed class MeasurementFactory : IDisposable
{
    ...
    public List<string> ProbeAll(bool? planUsesRtss = null, bool allowRtssStartup = true)
    {
        ...
        // Say WHICH reason applies. This used to always print "no RTSS game in this run plan", which is a
        // flat contradiction during full pre-flight on an RTSS roster — there RTSS is skipped because
        // startup is suppressed, not because no game wants it. Wrong-direction diagnostics in the suite's
        // own log are how an operator ends up debugging the wrong thing.
        if (_cfg.Osd && !rtssForOsd && !rtssForFrames)
            report.Add(planUsesRtss == true || !allowRtssStartup
                ? "[OSD]       skipped — RTSS startup is suppressed for this pass (full pre-flight never arms the global hook); the runtime starts it at the game boundary if the roster needs it."
                : "[OSD]       skipped — no RTSS game in this run plan, so RTSS (the global hook that crashes Ratchet) stays closed; the overlay renders only in RTSS-group sweeps.");
        ...
        // NOTE: no `|| IsRtssRunning()` here. It used to be, and it was dead code: with
        // allowRtssStartup:false, ShouldStartRtssDuringInitialProbe returns false at its first guard, so
        // rtssForFrames is false and rtssForOsd (which requires it) is false too — the left conjunct already
        // decided the branch. The XML doc's old claim that full pre-flight "may inspect an already-running
        // RTSS shared-memory mapping" was therefore untrue. Left deliberately not-inspected rather than made
        // to work: RtssLive feeding back into pre-flight would double-count with the separate
        // rtssReadyForLazyStart signal FullPreflightService already derives from the doctor rows, and that
        // is a verdict change that needs bench validation, not a dev-PC edit.
