...
public partial class HelpViewModel : ObservableObject
{
    ...
    [
        ...
        new("Run", "Full pre-flight and override", "Start automatically checks the enabled roster's FPS/frametime backend, FFmpeg vision path, qualified capture-card model and short stream, machine, every enabled game's installation, launcher login, update state, automation, and settings without launching a game. Known failures block the sweep; inconclusive online checks remain visible warnings. During a long campaign, each game is checked again immediately before its turn so a later logout or queued update cannot silently waste the roster. The advanced override only permits an attempted launch after review: it cannot manufacture frame evidence or bypass runtime validation, so an invalid run is still rejected.", "preflight blocker override fps frametime ffmpeg elgato roster"),
        new("Run", "FPS / frametime versus vision capture", "PresentMon or RTSS measures FPS and frametimes. FFmpeg plus the capture card supplies a separate image stream for menu OCR, vision navigation, and motion checks; it never supplies FPS. Full pre-flight derives which FPS backend is required from enabled game profiles. RTSS remains lazy and is never started merely because a future game might use it, protecting RTSS-forbidden titles.", "presentmon rtss fps frametime ffmpeg vision capture lazy ratchet"),
        new("Run", "Qualified capture card", "Elgato Game Capture 4K Pro is required for the full supported automated game-benchmark workflow and is the only capture-card model currently qualified by this project. Set Settings > Capture card device to the exact DirectShow name Elgato 4K Pro. Full pre-flight enumerates it through FFmpeg and runs a bounded stream probe; no image is retained. Elgato 4K60 Pro, MK.2, generic, or other cards are unqualified for that workflow, although offline authoring, builds, and diagnostic capture commands may still work. The 60 Hz safety setting describes this validated bench path; it is not a general product limit.", "elgato 4k pro capture card qualified 4k60 mk.2 diagnostics stream 60 hz"),
